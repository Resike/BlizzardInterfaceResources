Enum.AzeritePowerLevel.Base = 0
Enum.AzeritePowerLevel.Downgraded = 2
Enum.AzeritePowerLevel.Upgraded = 1
Enum.AzeritePowerLevelMeta.MaxValue = 2
Enum.AzeritePowerLevelMeta.MinValue = 0
Enum.AzeritePowerLevelMeta.NumValues = 3

Enum.BattlepayDisplayFlagMeta.MaxValue = 64
Enum.BattlepayDisplayFlagMeta.MinValue = 1
Enum.BattlepayDisplayFlagMeta.NumValues = 7

Enum.BattlepayGroupDisplayTypeMeta.MaxValue = 2
Enum.BattlepayGroupDisplayTypeMeta.MinValue = 0
Enum.BattlepayGroupDisplayTypeMeta.NumValues = 3

Enum.BattlepayProductDecoratorMeta.MaxValue = 3
Enum.BattlepayProductDecoratorMeta.MinValue = 0
Enum.BattlepayProductDecoratorMeta.NumValues = 4

Enum.BattlepayProductGroupFlagMeta.MaxValue = 16
Enum.BattlepayProductGroupFlagMeta.MinValue = 1
Enum.BattlepayProductGroupFlagMeta.NumValues = 5

Enum.BattlepaySpecialProductsMeta.MaxValue = 12
Enum.BattlepaySpecialProductsMeta.MinValue = 0
Enum.BattlepaySpecialProductsMeta.NumValues = 1

Enum.BrawlType.Arena = 2
Enum.BrawlType.Battleground = 1
Enum.BrawlType.Lfg = 3
Enum.BrawlType.None = 0

Enum.BrawlTypeMeta.MaxValue = 3
Enum.BrawlTypeMeta.MinValue = 0
Enum.BrawlTypeMeta.NumValues = 4

Enum.CalendarEventType.Dungeon = 1
Enum.CalendarEventType.HeroicDeprecated = 5
Enum.CalendarEventType.Meeting = 3
Enum.CalendarEventType.Other = 4
Enum.CalendarEventType.Pvp = 2
Enum.CalendarEventType.Raid = 0

Enum.CalendarEventTypeMeta.MaxValue = 5
Enum.CalendarEventTypeMeta.MinValue = 0
Enum.CalendarEventTypeMeta.NumValues = 6

Enum.CharacterServiceInfoFlag.RestrictToRecommendedSpecs = 1

Enum.CharacterServiceInfoFlagMeta.MaxValue = 1
Enum.CharacterServiceInfoFlagMeta.MinValue = 1
Enum.CharacterServiceInfoFlagMeta.NumValues = 1

Enum.ChatChannelType.Communities = 4
Enum.ChatChannelType.Custom = 1
Enum.ChatChannelType.None = 0
Enum.ChatChannelType.Private_Party = 2
Enum.ChatChannelType.Public_Party = 3

Enum.ClubActionType.ErrorClubActionAcceptInvitation = 13
Enum.ClubActionType.ErrorClubActionAddBan = 22
Enum.ClubActionType.ErrorClubActionCreate = 1
Enum.ClubActionType.ErrorClubActionCreateMessage = 24
Enum.ClubActionType.ErrorClubActionCreateStream = 15
Enum.ClubActionType.ErrorClubActionCreateTicket = 5
Enum.ClubActionType.ErrorClubActionDeclineInvitation = 14
Enum.ClubActionType.ErrorClubActionDestroy = 3
Enum.ClubActionType.ErrorClubActionDestroyMessage = 26
Enum.ClubActionType.ErrorClubActionDestroyStream = 17
Enum.ClubActionType.ErrorClubActionDestroyTicket = 6
Enum.ClubActionType.ErrorClubActionEdit = 2
Enum.ClubActionType.ErrorClubActionEditMember = 19
Enum.ClubActionType.ErrorClubActionEditMemberNote = 20
Enum.ClubActionType.ErrorClubActionEditMessage = 25
Enum.ClubActionType.ErrorClubActionEditStream = 16
Enum.ClubActionType.ErrorClubActionGetBans = 10
Enum.ClubActionType.ErrorClubActionGetInvitations = 11
Enum.ClubActionType.ErrorClubActionGetTicket = 8
Enum.ClubActionType.ErrorClubActionGetTickets = 9
Enum.ClubActionType.ErrorClubActionInviteMember = 18
Enum.ClubActionType.ErrorClubActionKickMember = 21
Enum.ClubActionType.ErrorClubActionLeave = 4
Enum.ClubActionType.ErrorClubActionRedeemTicket = 7
Enum.ClubActionType.ErrorClubActionRemoveBan = 23
Enum.ClubActionType.ErrorClubActionRevokeInvitation = 12
Enum.ClubActionType.ErrorClubActionSubscribe = 0

Enum.ClubActionTypeMeta.MaxValue = 26
Enum.ClubActionTypeMeta.MinValue = 0
Enum.ClubActionTypeMeta.NumValues = 27

Enum.ClubErrorType.ErrorClubAlreadyMember = 19
Enum.ClubErrorType.ErrorClubBanAlreadyExists = 35
Enum.ClubErrorType.ErrorClubBanCountAtMax = 36
Enum.ClubErrorType.ErrorClubFull = 16
Enum.ClubErrorType.ErrorClubInsufficientPrivileges = 24
Enum.ClubErrorType.ErrorClubInvalidRoleID = 23
Enum.ClubErrorType.ErrorClubInvitationAlreadyExists = 22
Enum.ClubErrorType.ErrorClubMemberHasRequiredRole = 31
Enum.ClubErrorType.ErrorClubNoClub = 17
Enum.ClubErrorType.ErrorClubNoSuchInvitation = 21
Enum.ClubErrorType.ErrorClubNoSuchMember = 20
Enum.ClubErrorType.ErrorClubNotMember = 18
Enum.ClubErrorType.ErrorClubReceivedInvitationCountAtMax = 33
Enum.ClubErrorType.ErrorClubSentInvitationCountAtMax = 32
Enum.ClubErrorType.ErrorClubStreamCountAtMax = 30
Enum.ClubErrorType.ErrorClubStreamCountAtMin = 29
Enum.ClubErrorType.ErrorClubStreamInvalidName = 28
Enum.ClubErrorType.ErrorClubStreamNoStream = 27
Enum.ClubErrorType.ErrorClubTargetIsBanned = 34
Enum.ClubErrorType.ErrorClubTicketCountAtMax = 37
Enum.ClubErrorType.ErrorClubTicketHasConsumedAllowedRedeemCount = 39
Enum.ClubErrorType.ErrorClubTicketNoSuchTicket = 38
Enum.ClubErrorType.ErrorClubTooManyClubsJoined = 25
Enum.ClubErrorType.ErrorClubVoiceFull = 26
Enum.ClubErrorType.ErrorCommunitiesBadTarget = 4
Enum.ClubErrorType.ErrorCommunitiesChatMute = 15
Enum.ClubErrorType.ErrorCommunitiesGuild = 8
Enum.ClubErrorType.ErrorCommunitiesIgnored = 7
Enum.ClubErrorType.ErrorCommunitiesMissingShortName = 11
Enum.ClubErrorType.ErrorCommunitiesNeutralFaction = 2
Enum.ClubErrorType.ErrorCommunitiesNone = 0
Enum.ClubErrorType.ErrorCommunitiesProfanity = 12
Enum.ClubErrorType.ErrorCommunitiesRestricted = 6
Enum.ClubErrorType.ErrorCommunitiesTrial = 13
Enum.ClubErrorType.ErrorCommunitiesUnknown = 1
Enum.ClubErrorType.ErrorCommunitiesUnknownRealm = 3
Enum.ClubErrorType.ErrorCommunitiesUnknownTicket = 10
Enum.ClubErrorType.ErrorCommunitiesVeteranTrial = 14
Enum.ClubErrorType.ErrorCommunitiesWrongFaction = 5
Enum.ClubErrorType.ErrorCommunitiesWrongRegion = 9

Enum.ClubErrorTypeMeta.MaxValue = 39
Enum.ClubErrorTypeMeta.MinValue = 0
Enum.ClubErrorTypeMeta.NumValues = 40

Enum.ClubFieldType.ClubBroadcast = 3
Enum.ClubFieldType.ClubDescription = 2
Enum.ClubFieldType.ClubName = 0
Enum.ClubFieldType.ClubShortName = 1
Enum.ClubFieldType.ClubStreamName = 4
Enum.ClubFieldType.ClubStreamSubject = 5
Enum.ClubFieldType.NumTypes = 6

Enum.ClubFieldTypeMeta.MaxValue = 6
Enum.ClubFieldTypeMeta.MinValue = 0
Enum.ClubFieldTypeMeta.NumValues = 7

Enum.ClubInvitationCandidateStatus.AlreadyMember = 2
Enum.ClubInvitationCandidateStatus.Available = 0
Enum.ClubInvitationCandidateStatus.InvitePending = 1

Enum.ClubInvitationCandidateStatusMeta.MaxValue = 2
Enum.ClubInvitationCandidateStatusMeta.MinValue = 0
Enum.ClubInvitationCandidateStatusMeta.NumValues = 3

Enum.ClubMemberPresence.Away = 4
Enum.ClubMemberPresence.Busy = 5
Enum.ClubMemberPresence.Offline = 3
Enum.ClubMemberPresence.Online = 1
Enum.ClubMemberPresence.OnlineMobile = 2
Enum.ClubMemberPresence.Unknown = 0

Enum.ClubMemberPresenceMeta.MaxValue = 5
Enum.ClubMemberPresenceMeta.MinValue = 0
Enum.ClubMemberPresenceMeta.NumValues = 6

Enum.ClubRemovedReason.Banned = 1
Enum.ClubRemovedReason.ClubDestroyed = 3
Enum.ClubRemovedReason.None = 0
Enum.ClubRemovedReason.Removed = 2

Enum.ClubRemovedReasonMeta.MaxValue = 3
Enum.ClubRemovedReasonMeta.MinValue = 0
Enum.ClubRemovedReasonMeta.NumValues = 4

Enum.ClubRestrictionReason.None = 0
Enum.ClubRestrictionReason.Unavailable = 1

Enum.ClubRestrictionReasonMeta.MaxValue = 1
Enum.ClubRestrictionReasonMeta.MinValue = 0
Enum.ClubRestrictionReasonMeta.NumValues = 2

Enum.ClubRoleIdentifier.Leader = 2
Enum.ClubRoleIdentifier.Member = 4
Enum.ClubRoleIdentifier.Moderator = 3
Enum.ClubRoleIdentifier.Owner = 1

Enum.ClubRoleIdentifierMeta.MaxValue = 4
Enum.ClubRoleIdentifierMeta.MinValue = 0
Enum.ClubRoleIdentifierMeta.NumValues = 4

Enum.ClubStreamNotificationFilter.All = 2
Enum.ClubStreamNotificationFilter.Mention = 1
Enum.ClubStreamNotificationFilter.None = 0

Enum.ClubStreamNotificationFilterMeta.MaxValue = 2
Enum.ClubStreamNotificationFilterMeta.MinValue = 0
Enum.ClubStreamNotificationFilterMeta.NumValues = 3

Enum.ClubStreamType.General = 0
Enum.ClubStreamType.Guild = 1
Enum.ClubStreamType.Officer = 2
Enum.ClubStreamType.Other = 3

Enum.ClubStreamTypeMeta.MaxValue = 3
Enum.ClubStreamTypeMeta.MinValue = 0
Enum.ClubStreamTypeMeta.NumValues = 4

Enum.ClubType.BattleNet = 0
Enum.ClubType.Character = 1
Enum.ClubType.Guild = 2
Enum.ClubType.Other = 3

Enum.ClubTypeMeta.MaxValue = 3
Enum.ClubTypeMeta.MinValue = 0
Enum.ClubTypeMeta.NumValues = 4

Enum.CommunicationMode.OpenMic = 1
Enum.CommunicationMode.PushToTalk = 0

Enum.CommunicationModeMeta.MaxValue = 1
Enum.CommunicationModeMeta.MinValue = 0
Enum.CommunicationModeMeta.NumValues = 2

Enum.ConsoleCategory.CategoryCombat = 3
Enum.ConsoleCategory.CategoryConsole = 2
Enum.ConsoleCategory.CategoryDebug = 0
Enum.ConsoleCategory.CategoryDefault = 5
Enum.ConsoleCategory.CategoryGame = 4
Enum.ConsoleCategory.CategoryGm = 8
Enum.ConsoleCategory.CategoryGraphics = 1
Enum.ConsoleCategory.CategoryNet = 6
Enum.ConsoleCategory.CategoryNone = 9
Enum.ConsoleCategory.CategorySound = 7

Enum.ConsoleCategoryMeta.MaxValue = 9
Enum.ConsoleCategoryMeta.MinValue = 0
Enum.ConsoleCategoryMeta.NumValues = 10

Enum.ConsoleColorType.AdminColor = 6
Enum.ConsoleColorType.BackgroundColor = 8
Enum.ConsoleColorType.ClickbufferColor = 9
Enum.ConsoleColorType.DefaultColor = 0
Enum.ConsoleColorType.DefaultGreen = 11
Enum.ConsoleColorType.EchoColor = 2
Enum.ConsoleColorType.ErrorColor = 3
Enum.ConsoleColorType.GlobalColor = 5
Enum.ConsoleColorType.HighlightColor = 7
Enum.ConsoleColorType.InputColor = 1
Enum.ConsoleColorType.PrivateColor = 10
Enum.ConsoleColorType.WarningColor = 4

Enum.ConsoleColorTypeMeta.MaxValue = 11
Enum.ConsoleColorTypeMeta.MinValue = 0
Enum.ConsoleColorTypeMeta.NumValues = 12

Enum.ConsoleCommandType.Command = 1
Enum.ConsoleCommandType.Cvar = 0
Enum.ConsoleCommandType.Script = 2

Enum.ConsoleCommandTypeMeta.MaxValue = 2
Enum.ConsoleCommandTypeMeta.MinValue = 0
Enum.ConsoleCommandTypeMeta.NumValues = 3

Enum.ContributionAppearanceFlags.TooltipUseTimeRemaining = 0

Enum.ContributionAppearanceFlagsMeta.MaxValue = 0
Enum.ContributionAppearanceFlagsMeta.MinValue = 0
Enum.ContributionAppearanceFlagsMeta.NumValues = 1

Enum.ContributionResult.FailedConditionCheck = 5
Enum.ContributionResult.IncorrectState = 2
Enum.ContributionResult.InternalError = 7
Enum.ContributionResult.InvalidID = 3
Enum.ContributionResult.MustBeNearNpc = 1
Enum.ContributionResult.QuestDataMissing = 4
Enum.ContributionResult.Success = 0
Enum.ContributionResult.UnableToCompleteTurnIn = 6

Enum.ContributionResultMeta.MaxValue = 7
Enum.ContributionResultMeta.MinValue = 0
Enum.ContributionResultMeta.NumValues = 8

Enum.ContributionState.Active = 2
Enum.ContributionState.Building = 1
Enum.ContributionState.Destroyed = 4
Enum.ContributionState.None = 0
Enum.ContributionState.UnderAttack = 3

Enum.ContributionStateMeta.MaxValue = 4
Enum.ContributionStateMeta.MinValue = 0
Enum.ContributionStateMeta.NumValues = 5

Enum.CustomBindingType.VoicePushToTalk = 0

Enum.CustomBindingTypeMeta.MaxValue = 0
Enum.CustomBindingTypeMeta.MinValue = 0
Enum.CustomBindingTypeMeta.NumValues = 1

Enum.FlightPathFaction.Alliance = 2
Enum.FlightPathFaction.Horde = 1
Enum.FlightPathFaction.Neutral = 0

Enum.FlightPathFactionMeta.MaxValue = 2
Enum.FlightPathFactionMeta.MinValue = 0
Enum.FlightPathFactionMeta.NumValues = 3

Enum.FlightPathState.Current = 0
Enum.FlightPathState.Reachable = 1
Enum.FlightPathState.Unreachable = 2

Enum.FlightPathStateMeta.MaxValue = 2
Enum.FlightPathStateMeta.MinValue = 0
Enum.FlightPathStateMeta.NumValues = 3

Enum.IconAndTextWidgetState.Hidden = 0
Enum.IconAndTextWidgetState.Shown = 1
Enum.IconAndTextWidgetState.ShownWithDynamicIconFlashing = 2
Enum.IconAndTextWidgetState.ShownWithDynamicIconNotFlashing = 3

Enum.IconAndTextWidgetStateMeta.MaxValue = 3
Enum.IconAndTextWidgetStateMeta.MinValue = 0
Enum.IconAndTextWidgetStateMeta.NumValues = 4

Enum.InventoryType.Index2HweaponType = 17
Enum.InventoryType.IndexAmmoType = 24
Enum.InventoryType.IndexBagType = 18
Enum.InventoryType.IndexBodyType = 4
Enum.InventoryType.IndexChestType = 5
Enum.InventoryType.IndexCloakType = 16
Enum.InventoryType.IndexFeetType = 8
Enum.InventoryType.IndexFingerType = 11
Enum.InventoryType.IndexHandType = 10
Enum.InventoryType.IndexHeadType = 1
Enum.InventoryType.IndexHoldableType = 23
Enum.InventoryType.IndexLegsType = 7
Enum.InventoryType.IndexNeckType = 2
Enum.InventoryType.IndexNonEquipType = 0
Enum.InventoryType.IndexQuiverType = 27
Enum.InventoryType.IndexRangedrightType = 26
Enum.InventoryType.IndexRangedType = 15
Enum.InventoryType.IndexRelicType = 28
Enum.InventoryType.IndexRobeType = 20
Enum.InventoryType.IndexShieldType = 14
Enum.InventoryType.IndexShoulderType = 3
Enum.InventoryType.IndexTabardType = 19
Enum.InventoryType.IndexThrownType = 25
Enum.InventoryType.IndexTrinketType = 12
Enum.InventoryType.IndexWaistType = 6
Enum.InventoryType.IndexWeaponmainhandType = 21
Enum.InventoryType.IndexWeaponoffhandType = 22
Enum.InventoryType.IndexWeaponType = 13
Enum.InventoryType.IndexWristType = 9

Enum.InventoryTypeMeta.MaxValue = 28
Enum.InventoryTypeMeta.MinValue = 0
Enum.InventoryTypeMeta.NumValues = 29

Enum.ItemQuality.Artifact = 6
Enum.ItemQuality.Epic = 4
Enum.ItemQuality.Good = 2
Enum.ItemQuality.Heirloom = 7
Enum.ItemQuality.Legendary = 5
Enum.ItemQuality.Poor = 0
Enum.ItemQuality.Standard = 1
Enum.ItemQuality.Superior = 3
Enum.ItemQuality.WoWToken = 8

Enum.ItemQualityMeta.MaxValue = 8
Enum.ItemQualityMeta.MinValue = 0
Enum.ItemQualityMeta.NumValues = 9

Enum.ItemTryOnReason.DataPending = 3
Enum.ItemTryOnReason.NotEquippable = 2
Enum.ItemTryOnReason.Success = 0
Enum.ItemTryOnReason.WrongRace = 1
Enum.ItemTryOnReasonMeta.MaxValue = 3
Enum.ItemTryOnReasonMeta.MinValue = 0
Enum.ItemTryOnReasonMeta.NumValues = 4

Enum.ManipulatorEventType.Complete = 2
Enum.ManipulatorEventType.Delete = 3
Enum.ManipulatorEventType.Move = 1
Enum.ManipulatorEventType.Start = 0

Enum.ManipulatorEventTypeMeta.MaxValue = 3
Enum.ManipulatorEventTypeMeta.MinValue = 0
Enum.ManipulatorEventTypeMeta.NumValues = 4

Enum.MapCanvasPosition.BottomLeft = 1
Enum.MapCanvasPosition.BottomRight = 2
Enum.MapCanvasPosition.None = 0
Enum.MapCanvasPosition.TopLeft = 3
Enum.MapCanvasPosition.TopRight = 4

Enum.MapCanvasPositionMeta.MaxValue = 4
Enum.MapCanvasPositionMeta.MinValue = 0
Enum.MapCanvasPositionMeta.NumValues = 5

Enum.ModelSceneSetting.AlignLightToOrbitDelta = 1

Enum.ModelSceneSettingMeta.MaxValue = 1
Enum.ModelSceneSettingMeta.MinValue = 1
Enum.ModelSceneSettingMeta.NumValues = 1

Enum.ModelSceneType.ArtifactRelicTalentEffect = 9
Enum.ModelSceneType.ArtifactTier2 = 5
Enum.ModelSceneType.ArtifactTier2ForgingScene = 6
Enum.ModelSceneType.ArtifactTier2SlamEffect = 7
Enum.ModelSceneType.AzeriteItemLevelUpToast = 13
Enum.ModelSceneType.AzeritePowers = 14
Enum.ModelSceneType.AzeriteRewardGlow = 15
Enum.ModelSceneType.CommentatorVictoryFanfare = 8
Enum.ModelSceneType.EncounterJournal = 3
Enum.ModelSceneType.MountJournal = 0
Enum.ModelSceneType.PartyPose = 12
Enum.ModelSceneType.PetJournalCard = 1
Enum.ModelSceneType.PetJournalLoadout = 4
Enum.ModelSceneType.PvpWarModeFire = 11
Enum.ModelSceneType.PvpWarModeOrb = 10
Enum.ModelSceneType.ShopCard = 2

Enum.ModelSceneTypeMeta.MaxValue = 15
Enum.ModelSceneTypeMeta.MinValue = 0
Enum.ModelSceneTypeMeta.NumValues = 16

Enum.PartyRequestJoinRelation.Club = 3
Enum.PartyRequestJoinRelation.Friend = 1
Enum.PartyRequestJoinRelation.Guild = 2
Enum.PartyRequestJoinRelation.None = 0

Enum.PartyRequestJoinRelation.NumPartyRequestJoinRelations = 4
Enum.PartyRequestJoinRelationMeta.MaxValue = 4
Enum.PartyRequestJoinRelationMeta.MinValue = 0
Enum.PartyRequestJoinRelationMeta.NumValues = 5

Enum.PerfActivity.ActivityAnimate = 15
Enum.PerfActivity.ActivityEventnet = 53
Enum.PerfActivity.ActivityModelRender = 11
Enum.PerfActivity.ActivitySwap = 36
Enum.PerfActivity.ActivityUnit = 45
Enum.PerfActivity.ActivityWorldLink = 6

Enum.PerfActivityMeta.MaxValue = 5
Enum.PerfActivityMeta.MinValue = 0
Enum.PerfActivityMeta.NumValues = 6

Enum.PerfCounter.PerfCountBatchCount = 22
Enum.PerfCounter.PerfCountPrimitiveCount = 17
Enum.PerfCounter.PerfCountUnits = 3

Enum.PerfCounterMeta.MaxValue = 2
Enum.PerfCounterMeta.MinValue = 0
Enum.PerfCounterMeta.NumValues = 3

Enum.PowerType.Alternate = 10
Enum.PowerType.ArcaneCharges = 16
Enum.PowerType.Chi = 12
Enum.PowerType.ComboPoints = 4
Enum.PowerType.Energy = 3
Enum.PowerType.Focus = 2
Enum.PowerType.Fury = 17
Enum.PowerType.HealthCost = -2
Enum.PowerType.HolyPower = 9
Enum.PowerType.Insanity = 13
Enum.PowerType.LunarPower = 8
Enum.PowerType.Maelstrom = 11
Enum.PowerType.Mana = 0
Enum.PowerType.None = -1
Enum.PowerType.NumPowerTypes = 19
Enum.PowerType.Obsolete = 14
Enum.PowerType.Obsolete2 = 15
Enum.PowerType.Pain = 18
Enum.PowerType.Rage = 1
Enum.PowerType.Runes = 5
Enum.PowerType.RunicPower = 6
Enum.PowerType.SoulShards = 7

Enum.PowerTypeMeta.MaxValue = 19
Enum.PowerTypeMeta.MinValue = -2
Enum.PowerTypeMeta.NumValues = 22

Enum.QuestLineFloorLocation.Above = 0
Enum.QuestLineFloorLocation.Below = 1
Enum.QuestLineFloorLocation.Same = 2

Enum.QuestLineFloorLocationMeta.MaxValue = 2
Enum.QuestLineFloorLocationMeta.MinValue = 0
Enum.QuestLineFloorLocationMeta.NumValues = 3

Enum.QuestTag.Account = 102
Enum.QuestTag.Dungeon = 81
Enum.QuestTag.Group = 1
Enum.QuestTag.Heroic = 85
Enum.QuestTag.Legendary = 83
Enum.QuestTag.Pvp = 41
Enum.QuestTag.Raid = 62
Enum.QuestTag.Raid10 = 88
Enum.QuestTag.Raid25 = 89
Enum.QuestTag.Scenario = 98

Enum.QuestTagMeta.MaxValue = 102
Enum.QuestTagMeta.MinValue = 0
Enum.QuestTagMeta.NumValues = 10

Enum.SelfResurrectOptionType.Item = 1
Enum.SelfResurrectOptionType.Spell = 0

Enum.SelfResurrectOptionTypeMeta.MaxValue = 1
Enum.SelfResurrectOptionTypeMeta.MinValue = 0
Enum.SelfResurrectOptionTypeMeta.NumValues = 2

Enum.SpellDisplayIconSizeType.Large = 2
Enum.SpellDisplayIconSizeType.Medium = 1
Enum.SpellDisplayIconSizeType.Small = 0

Enum.SpellDisplayIconSizeTypeMeta.MaxValue = 2
Enum.SpellDisplayIconSizeTypeMeta.MinValue = 0
Enum.SpellDisplayIconSizeTypeMeta.NumValues = 3

Enum.StatusBarValueTextType.Hidden = 0
Enum.StatusBarValueTextType.Percentage = 1
Enum.StatusBarValueTextType.Time = 3
Enum.StatusBarValueTextType.TimeShowOneLevelOnly = 4
Enum.StatusBarValueTextType.Value = 2
Enum.StatusBarValueTextType.ValueOverMax = 5

Enum.StatusBarValueTextTypeMeta.MaxValue = 5
Enum.StatusBarValueTextTypeMeta.MinValue = 0
Enum.StatusBarValueTextTypeMeta.NumValues = 6

Enum.StoreDeliveryType.Battlepet = 2
Enum.StoreDeliveryType.Collection = 3
Enum.StoreDeliveryType.Item = 0
Enum.StoreDeliveryType.Mount = 1

Enum.StoreDeliveryTypeMeta.MaxValue = 3
Enum.StoreDeliveryTypeMeta.MinValue = 0
Enum.StoreDeliveryTypeMeta.NumValues = 4

Enum.StoreErrorMeta.MaxValue = 11
Enum.StoreErrorMeta.MinValue = 0
Enum.StoreErrorMeta.NumValues = 12

Enum.SummonStatus.Accepted = 2
Enum.SummonStatus.Declined = 3
Enum.SummonStatus.None = 0
Enum.SummonStatus.Pending = 1

Enum.SummonStatusMeta.MaxValue = 3
Enum.SummonStatusMeta.MinValue = 0
Enum.SummonStatusMeta.NumValues = 4

Enum.TransmogSource.Achievement = 7
Enum.TransmogSource.CantCollect = 6
Enum.TransmogSource.HiddenUntilCollected = 5
Enum.TransmogSource.JournalEncounter = 1
Enum.TransmogSource.None = 0
Enum.TransmogSource.NotValidForTransmog = 9
Enum.TransmogSource.Profession = 8
Enum.TransmogSource.Quest = 2
Enum.TransmogSource.Vendor = 3
Enum.TransmogSource.WorldDrop = 4

Enum.TransmogSourceMeta.MaxValue = 9
Enum.TransmogSourceMeta.MinValue = 0
Enum.TransmogSourceMeta.NumValues = 10

Enum.UIMapSystem.Adventure = 2
Enum.UIMapSystem.Taxi = 1
Enum.UIMapSystem.World = 0

Enum.UIMapSystemMeta.MaxValue = 2
Enum.UIMapSystemMeta.MinValue = 0
Enum.UIMapSystemMeta.NumValues = 3

Enum.UIMapType.Continent = 2
Enum.UIMapType.Cosmic = 0
Enum.UIMapType.Dungeon = 4
Enum.UIMapType.Micro = 5
Enum.UIMapType.Orphan = 6
Enum.UIMapType.World = 1
Enum.UIMapType.Zone = 3

Enum.UIMapTypeMeta.MaxValue = 6
Enum.UIMapTypeMeta.MinValue = 0
Enum.UIMapTypeMeta.NumValues = 7

Enum.UIWidgetVisualizationType.BulletTextList = 10
Enum.UIWidgetVisualizationType.CaptureBar = 1
Enum.UIWidgetVisualizationType.DoubleIconAndText = 5
Enum.UIWidgetVisualizationType.DoubleStatusBar = 3
Enum.UIWidgetVisualizationType.HorizontalCurrencies = 9
Enum.UIWidgetVisualizationType.IconAndText = 0
Enum.UIWidgetVisualizationType.IconTextAndBackground = 4
Enum.UIWidgetVisualizationType.IconTextAndCurrencies = 7
Enum.UIWidgetVisualizationType.ScenarioHeaderCurrenciesAndBackground = 11
Enum.UIWidgetVisualizationType.SpellDisplay = 13
Enum.UIWidgetVisualizationType.StackedResourceTracker = 6
Enum.UIWidgetVisualizationType.StatusBar = 2
Enum.UIWidgetVisualizationType.TextureWithState = 12
Enum.UIWidgetVisualizationType.TextWithState = 8

Enum.UIWidgetVisualizationTypeMeta.MaxValue = 13
Enum.UIWidgetVisualizationTypeMeta.MinValue = 0
Enum.UIWidgetVisualizationTypeMeta.NumValues = 14

Enum.ValidateNameResult.NameConsecutiveSpaces = 13
Enum.ValidateNameResult.NameDeclensionDoesntMatchBaseName = 16
Enum.ValidateNameResult.NameFailure = 1
Enum.ValidateNameResult.NameInvalidApostrophe = 9
Enum.ValidateNameResult.NameInvalidCharacter = 5
Enum.ValidateNameResult.NameInvalidSpace = 12
Enum.ValidateNameResult.NameMixedLanguages = 6
Enum.ValidateNameResult.NameMultipleApostrophes = 10
Enum.ValidateNameResult.NameNoName = 2
Enum.ValidateNameResult.NameProfane = 7
Enum.ValidateNameResult.NameReserved = 8
Enum.ValidateNameResult.NameRussianConsecutiveSilentCharacters = 14
Enum.ValidateNameResult.NameRussianSilentCharacterAtBeginningOrEnd = 15
Enum.ValidateNameResult.NameSuccess = 0
Enum.ValidateNameResult.NameThreeConsecutive = 11
Enum.ValidateNameResult.NameTooLong = 4
Enum.ValidateNameResult.NameTooShort = 3

Enum.ValidateNameResultMeta.MaxValue = 16
Enum.ValidateNameResultMeta.MinValue = 0
Enum.ValidateNameResultMeta.NumValues = 17

Enum.VasErrorMeta.MaxValue = 20086
Enum.VasErrorMeta.MinValue = 0
Enum.VasErrorMeta.NumValues = 39

Enum.VasPurchaseProgress.ApplyingLicense = 3
Enum.VasPurchaseProgress.Complete = 7
Enum.VasPurchaseProgress.Invalid = 0
Enum.VasPurchaseProgress.PaymentPending = 2
Enum.VasPurchaseProgress.PrePurchase = 1
Enum.VasPurchaseProgress.ProcessingFactionChange = 6
Enum.VasPurchaseProgress.Ready = 5
Enum.VasPurchaseProgress.WaitingOnQueue = 4

Enum.VasPurchaseProgressMeta.MaxValue = 7
Enum.VasPurchaseProgressMeta.MinValue = 0
Enum.VasPurchaseProgressMeta.NumValues = 8

Enum.VasServiceTypeMeta.MaxValue = 9
Enum.VasServiceTypeMeta.MinValue = 0
Enum.VasServiceTypeMeta.NumValues = 10

Enum.VignetteType.Normal = 0
Enum.VignetteType.PvpBounty = 1

Enum.VignetteTypeMeta.MaxValue = 1
Enum.VignetteTypeMeta.MinValue = 0
Enum.VignetteTypeMeta.NumValues = 2

Enum.VoiceChatStatusCode.AlreadyInChannel = 10
Enum.VoiceChatStatusCode.ChannelAlreadyExists = 9
Enum.VoiceChatStatusCode.ChannelNameTooLong = 8
Enum.VoiceChatStatusCode.ChannelNameTooShort = 7
Enum.VoiceChatStatusCode.ClientAlreadyLoggedIn = 6
Enum.VoiceChatStatusCode.ClientNotInitialized = 4
Enum.VoiceChatStatusCode.ClientNotLoggedIn = 5
Enum.VoiceChatStatusCode.Disabled = 18
Enum.VoiceChatStatusCode.Failure = 12
Enum.VoiceChatStatusCode.InvalidCommunityStream = 20
Enum.VoiceChatStatusCode.LoginProhibited = 3
Enum.VoiceChatStatusCode.OperationPending = 1
Enum.VoiceChatStatusCode.PlayerSilenced = 21
Enum.VoiceChatStatusCode.PlayerVoiceChatParentalDisabled = 22
Enum.VoiceChatStatusCode.ProxyConnectionTimeOut = 15
Enum.VoiceChatStatusCode.ProxyConnectionUnableToConnect = 16
Enum.VoiceChatStatusCode.ProxyConnectionUnexpectedDisconnect = 17
Enum.VoiceChatStatusCode.ServiceLost = 13
Enum.VoiceChatStatusCode.Success = 0
Enum.VoiceChatStatusCode.TargetNotFound = 11
Enum.VoiceChatStatusCode.TooManyRequests = 2
Enum.VoiceChatStatusCode.UnableToLaunchProxy = 14
Enum.VoiceChatStatusCode.UnsupportedChatChannelType = 19

Enum.VoiceChatStatusCodeMeta.MaxValue = 22
Enum.VoiceChatStatusCodeMeta.MinValue = 0
Enum.VoiceChatStatusCodeMeta.NumValues = 23

Enum.WidgetEnabledState.Disabled = 0
Enum.WidgetEnabledState.Enabled = 1
Enum.WidgetEnabledState.Highlight = 3
Enum.WidgetEnabledState.Red = 2

Enum.WidgetEnabledStateMeta.MaxValue = 3
Enum.WidgetEnabledStateMeta.MinValue = 0
Enum.WidgetEnabledStateMeta.NumValues = 4

Enum.WidgetShownState.Hidden = 0
Enum.WidgetShownState.Shown = 1

Enum.WidgetShownStateMeta.MaxValue = 1
Enum.WidgetShownStateMeta.MinValue = 0
Enum.WidgetShownStateMeta.NumValues = 2

Enum.ZoneAbilityType.Argus = 2
Enum.ZoneAbilityType.Garrison = 0
Enum.ZoneAbilityType.OrderHall = 1
Enum.ZoneAbilityType.WarEffort = 3

Enum.ZoneAbilityTypeMeta.MaxValue = 3
Enum.ZoneAbilityTypeMeta.MinValue = 0
Enum.ZoneAbilityTypeMeta.NumValues = 4